...
@ConnectorAttribute(name = "max.poll.buffer.size", type = "int", direction = Direction.INCOMING, description = "The maximum number of polled messages to buffer waiting to be consumed. This is used to limit the memory consumption while implementing back-pressure with `pause-if-no-requests`. Defaults to `max.poll.records` times 5.")
